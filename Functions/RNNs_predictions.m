...
for i_pos=1:Npos
    for i_dir=1:Ndir
        ...
        for i_cycle=1:Ncycle
            ...
            %% For shared variance during prep
            FRicycle=states(this_cond(this_prep),:);
            idxCycle_prep=[idxCycle_prep;ones(numel(this_cond(this_prep)),1)*i_cycle];
        ...
        %% PCA prep
        FRicycle_prep_all_all=[FRicycle_prep_all_all;FRicycle_prep_all];
        idxCycle_prep_all=[idxCycle_prep_all;idxCycle_prep];
        idxDir_prep_all=[idxDir_prep_all;idxDir_prep];
        idxPos_prep_all=[idxPos_prep_all;idxPos_prep];
...
if do_plot_pred
    [~,PCAPrep]=pca(FRicycle_prep_all_all);

    for i_pos=1:Npos
        for i_dir=1:Ndir
            for i_cycle=1:Ncycle
                this_cond=find(idxCycle_prep_all==i_cycle & idxDir_prep_all==i_dir & idxPos_prep_all==i_pos);
...
for i_dir=1:Ndir
    for i_pos=1:Npos
        ...
        if do_plot && i_dir==1 && i_pos==1
            ...
            figure
            % Add prep for context
            %Rhythmic=idx_cycle==7 & idx_dir==i_dir & idx_pos==i_pos;
            ...
            %exec_rhyth=find(idx_Ncycle(Rhythmic)>1 & idx_Ncycle(Rhythmic)<7);
            plot3(scores(NelD+1:end,1),scores(NelD+1:end,2),scores(NelD+1:end,3),'Color',colour_cycle(5,:),'LineWidth',2)
            %plot3(scores(NelD+exec_rhyth,1),scores(NelD+exec_rhyth,2),scores(NelD+exec_rhyth,3),'r','LineWidth',2)
            title(num2str(Angle_dir(i_dir,i_pos),'%.2f'))


            %% plot rotational plane rhtyhmic
            %plot_rot_plane(summary.jPCs_highD(:,1:2),coeffs(:,1:3),scores(NelD+1:end,1:3),colour_cycle(5,:))
            %plot_rot_plane(summary2.jPCs_highD(:,1:2),coeffs(:,1:3),scores(exec_disc,1:3),colour_cycle(1,:))
            %             rot_plane=summary.jPCs_highD(:,1:2)'*coeffs(:,1:3);
            %             a=cross(rot_plane(1,:)',rot_plane(2,:)');
            %             minax= min(scores(NelD+1:end,1:2),'all');
            %             maxax= max(scores(NelD+1:end,1:2),'all');
            %
            %             [x,y] = meshgrid(-minax:0.01:maxax);
            %             z=(-a(1)*x-a(2)*y)/a(3);
...
Ncycles=unique(idx_cycle); % hardcoding
...
for i_dir=1:Ndir
    for i_pos=1:Npos
        ...
        Rhythmic=idx_cycle==7 & idx_Ncycle>1 & idx_Ncycle<7 & idx_dir==i_dir & idx_pos==i_pos;
        %%
        % Compute the natural variability of the neural activity during the
