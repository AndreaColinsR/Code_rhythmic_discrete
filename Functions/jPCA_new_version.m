...
%  That way you can make a principled decision regarding how tAndreao treat the original data versus the
...
% 
%% This function was edited by Andrea Colins to make it suitable for the manuscript
% All the edits are documented by comments starting with Andrea
%% 17/12/2025
function [Projection, Summary] = jPCA_new_version(Data, analyzeTimes, params)
...
% Andrea deleted the following mild warning since we used recordings with
% dt = 1 ms.
...
%Andrea changed this because 'princomp' has been removed
...
function Vr = getRealVs(V,evals)
    ...
    %% Andrea changed the line below because for Aplysia we only have one condition only
    testProj = (Vr'*Ared(1:end,:)')'; % just picks out the plan times
    %testProj = (Vr'*Ared(1:numAnalyzedTimes:end,:)')'; % just picks out the plan times
    %Andrea changes this because 'princomp' has been removed
    rotV = pca(testProj);
