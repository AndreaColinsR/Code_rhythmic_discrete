function [corr_CC,Angle_rotRNN,OutW,MinDist,Dist_prep_onset]=Evaluating_family_of_RNNs(hyp,region_name,my_dir,plot_supp,varargin)

if nargin==7
    fig_supp_prep=varargin{3};
    ifamily=varargin{2};
    figW=varargin{1};
elseif nargin==6
    ifamily=varargin{2};
...
for i_animal=1:Nanimal
    ...
    Results = Evaluate_this_animal(animal{i_animal},hyp,figW,ifamily,scores,cond_idx_M1,region_name,figW2,plot_supp,fig_supp_prep);
...
function Results = Evaluate_this_animal(animal,hyp,figW,ifamily,scores_M1,cond_idx_M1,region_name,figW2,plot_supp,fig_supp_prep)
...
for iNet=1:Nnetworks
    ...
    [scores,trials_idx,R2(iNet,:),states,Output_edited,Output_RNN,r_end(iNet)] = Eval_RNN_all_conditions(Input,Output,Test_input,Test_Outputs,net_params,exec,idx_conditions_train,idx_conditions_test,do_plot_output);
    ...
    if R2(iNet,2)>=0.8
        ...
        [Angle_disc_rhyth(iNet),Init_cond_t(start:start+3,:),Dist2Att]=RNNs_predictions(states,idx_dir,idx_pos,idx_dist,exec2,idx_Ncycle,do_plot_pred,column,fig_supp_prep);
        start=start+4;
        ...
        [~,Percentage_all_prep(iNet,:)]=dPCA_across_conditions(prepdata.FR-mean(prepdata.FR),prepdata.ndir,prepdata.npos,prepdata.ndist,do_plot,column);
