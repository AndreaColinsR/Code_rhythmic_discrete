...
def simulateRNN(Bipos,Bdir,Btask,B,W,O,Ob,I,tau):
    """
    Simulate the trained RNN dynamics for a single trial 
    ...
    Btask : array, shape (2, Nunits)
        Fixed input projection weights for movement type (discrete vs rhythmic).
    ...
    I : array, shape (1, Ntimes, Ninputs)
        Input time series for one trial. In this notebook, Ninputs = 7:
            - 2 start position inputs
            - 1 temporal context / stop input
            - 2 direction inputs
            - 2 movement-type inputs (discrete vs rhythmic)
    tau : float
        Effective time constant (in discrete steps).
    ...
    state : array, shape (Nunits, Ntimes+1)
        Simulated hidden state trajectory (includes initial state at index 0).
    """
    Ntimes=I.shape[1]
    Nunits=W.shape[0]
    ...
    for t1 in np.arange(0,Ntimes):
        ...
        It= np.matmul(I[0,t1:t1+1,0:2],Bipos[0:2,:])+\
            B[0,:].transpose()*I[0,t1:t1+1,2:3]+\
            np.matmul(I[0,t1:t1+1,3:5],Bdir[0:2,:])+\
            np.matmul(I[0,t1:t1+1,5:7],Btask[0:2,:])
